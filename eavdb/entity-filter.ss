  ...
  (string-append
   ...
   (if (not (equal? typed "*"))
       (if (equal? typed "mongoose")
           "where e.entity_type = ? and (d.value = 0 or d.value is NULL or d.value='NULL') order by substr(n.value,3)"
           "where e.entity_type = ? and (d.value = 0 or d.value is NULL or d.value='NULL') order by n.value")
       "order by n.value")))
